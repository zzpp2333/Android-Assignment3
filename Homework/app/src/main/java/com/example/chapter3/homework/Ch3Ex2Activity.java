...
public class Ch3Ex2Activity extends AppCompatActivity {
    ...
    private void resetTargetAnimation() {
        ...
        ObjectAnimator animatoralp = ObjectAnimator.ofFloat(target,
                "alpha",1.0f,0.5f);
        animatoralp.setInterpolator(new LinearInterpolator());
        animatoralp.setDuration(Integer.parseInt(durationSelector.getText().toString()));
        animatoralp.setRepeatCount(ObjectAnimator.INFINITE);
        animatoralp.setRepeatMode(ValueAnimator.REVERSE);

        // TODO ex2-3: 将上面创建的其他 ObjectAnimator 都添加到 AnimatorSet 中
        animatorSet = new AnimatorSet();
        animatorSet.playTogether(animator1,animator2x,animator2y,animatoralp);
        animatorSet.start();
    }
}
